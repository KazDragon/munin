...
{
  std::shared_ptr<munin::framed_component> comp =
      munin::make_framed_component(make_mock_frame(), munin::make_fill('X'));
}
...
TEST(a_zero_size_framed_component, positions_subcomponents_at_origin)
{
  ...
  auto framed_component = munin::make_framed_component(mock_frame, mock_comp);
...
TEST(
    a_zero_height_framed_component,
    positions_inner_component_in_an_inner_column)
{
...
TEST(
    a_framed_component_larger_than_the_frame,
    positions_the_inner_component_inward_of_the_frame)
{
  std::shared_ptr<mock_frame> mock_frame = make_mock_frame();
  std::shared_ptr<mock_component> mock_comp = make_mock_component();
...
TEST(
    a_framed_component,
    has_a_preferred_size_of_the_sum_of_the_frame_borders_and_inner)
{
  std::shared_ptr<mock_frame> mock_frame = make_mock_frame();
  std::shared_ptr<mock_component> mock_comp = make_mock_component();
  ...
  ASSERT_EQ(terminalpp::extent(14, 16), framed_component->get_preferred_size());
}
...
TEST(a_framed_component, draws_the_inner_component_inside_the_frame)
{
  ...
  std::shared_ptr<munin::framed_component> comp = munin::make_framed_component(
      munin::make_solid_frame(), munin::make_fill({'X', fill_attribute}));
