...
{
  return std::make_shared<testing::NiceMock<mock_frame_with_focus_attribute>>();
}
...
class a_framed_component_with_an_inner_component : public testing::Test
{
 protected:
  a_framed_component_with_an_inner_component()
  {
    ...
    ON_CALL(*mock_frame_, north_border_height()).WillByDefault(Return(1));
    ON_CALL(*mock_frame_, south_border_height()).WillByDefault(Return(1));
    ON_CALL(*mock_frame_, west_border_width()).WillByDefault(Return(1));
    ON_CALL(*mock_frame_, east_border_width()).WillByDefault(Return(1));
        ...
        .WillByDefault(
            [this](auto &surface, auto const &region)
            {
              auto const &focus_element = terminalpp::element{
                  frame_glyph, mock_frame_->get_frame_focus_attribute()};
...
TEST_F(
    a_framed_component_with_an_inner_component,
    has_the_lowlight_attribute_when_the_inner_component_does_not_have_focus)
{
  framed_component_->draw(surface_, {{}, framed_component_->get_size()});

  auto const expected_element =
      terminalpp::element{frame_glyph, lowlight_attribute};
...
TEST_F(
    a_framed_component_with_an_inner_component,
    has_the_highlight_attribute_when_the_inner_component_has_focus)
{
  ON_CALL(*mock_inner_, do_has_focus()).WillByDefault(Return(true));

  framed_component_->draw(surface_, {{}, framed_component_->get_size()});

  auto const expected_element =
      terminalpp::element{frame_glyph, highlight_attribute};
