...
TEST_F(
    a_text_area_with_text_inserted,
    can_have_text_inserted_that_pushes_existing_text_into_the_next_line)
{
    text_area_.set_size({4, 2});
...
TEST_F(
    a_text_area_with_text_inserted,
    announces_a_change_in_preferred_size_if_its_size_is_changed_such_that_there_are_now_more_lines)
{
...
TEST_P(
    setting_the_caret_programatically, moves_the_caret_and_cursor_as_specified)
{
    using std::get;
    ...
    text_area_.on_cursor_position_changed.connect([this, &cursor_position] {
    ...
    auto const character = caret_position == text.size()
                             ? '!'
...
TEST_P(
    setting_the_cursor_programatically, moves_the_caret_and_cursor_as_specified)
{
    using std::get;
    ...
    auto const text = text_area_.get_text();
    auto const character = expected_caret_position == text.size()
                             ? '!'
...
TEST_F(
    ...
    moves_the_cursor_to_where_the_mouse_clicks)
{
