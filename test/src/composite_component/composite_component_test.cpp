...
public:
    ...
    std::shared_ptr<mock_component> inner_component {
        make_mock_component()
    };
};
...
{
protected:
    std::shared_ptr<composite_mock> composite {
...
{
    {
        ...
        EXPECT_CALL(*composite->inner_component, do_get_cursor_state())
    ...
    // cursor state.
    composite->set_focus();
    ON_CALL(*composite->inner_component, do_has_focus)
        .WillByDefault(Return(true));

    ASSERT_FALSE(composite->get_cursor_state());
    ASSERT_TRUE(composite->get_cursor_state());
}
...
TEST_F(a_composite_component, returns_the_inner_components_cursor_position)
{
    {
        testing::InSequence seq;
        EXPECT_CALL(*composite->inner_component, do_get_cursor_position())
    ...
    ASSERT_EQ(terminalpp::point(1, 1), composite->get_cursor_position());
    ASSERT_EQ(terminalpp::point(2, 2), composite->get_cursor_position());
}

TEST_F(a_composite_component, sets_the_cursor_position_of_its_inner_component)
{
    static constexpr terminalpp::point pos{2, 7};
    EXPECT_CALL(*composite->inner_component, do_set_cursor_position(pos));

    composite->set_focus();
    ON_CALL(*composite->inner_component, do_has_focus)
        .WillByDefault(Return(true));

    composite->set_cursor_position(pos);
}
...
TEST_F(a_composite_component, forwards_events_to_its_inner_component)
{
    ...
    EXPECT_CALL(*composite->inner_component, do_event(_))
