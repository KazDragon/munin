...
public:
    a_new_scroll_pane()
    {
        ...
        ON_CALL(*inner_component_, do_set_position(_))
            .WillByDefault(SaveArg<0>(&mock_position_));

        ON_CALL(*inner_component_, do_get_position())
            .WillByDefault(ReturnPointee(&mock_position_));

        ON_CALL(*inner_component_, do_set_size(_))
            .WillByDefault(SaveArg<0>(&mock_size_));

        ON_CALL(*inner_component_, do_get_size())
            .WillByDefault(ReturnPointee(&mock_size_));
    }

protected:
    terminalpp::point mock_position_;
    terminalpp::extent mock_size_;
    terminalpp::point cursor_position_;
    std::shared_ptr<mock_component> inner_component_{make_mock_component()};

    std::shared_ptr<munin::scroll_pane> scroll_pane_{
    ...
    highlights_the_whole_frame_when_the_underlying_component_receives_focus)
{
    ...
    scroll_pane_->set_size({4, 4});
    scroll_pane_->set_highlight_attribute(highlight_attribute);
    ...
    lowlights_the_whole_frame_when_the_underlying_component_loses_focus)
{
    ...
    scroll_pane_->set_size({4, 4});
    scroll_pane_->set_highlight_attribute(highlight_attribute);
    scroll_pane_->set_lowlight_attribute(lowlight_attribute);
