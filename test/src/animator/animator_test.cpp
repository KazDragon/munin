...
class an_animator : public testing::Test
{
 protected:
  an_animator()
    : now_(std::chrono::steady_clock::now()), redraw_requested_(false)
  {
    ON_CALL(*animator_, do_now()).WillByDefault(Return(now_));

    component_->on_redraw.connect([this](auto)
                                  { this->redraw_requested_ = true; });
  }
  ...
  : public testing::Test
{
 protected:
      ...
      redraw1_requested_(false)
  {
    ...
    component0_->on_redraw.connect([this](auto)
                                   { this->redraw0_requested_ = true; });

    bool redraw1_requested = false;
    component1_->on_redraw.connect([this](auto)
                                   { this->redraw1_requested_ = true; });
