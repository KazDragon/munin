...
TEST(grid_layout_test, grid_layout_is_layout)
{
  std::unique_ptr<munin::layout> lyt =
      munin::make_grid_layout(terminalpp::extent{1, 1});
}
...
TEST(
    grid_layout_test,
    preferred_size_of_a_1x1_grid_with_one_view_is_the_preferred_size_of_the_view)
{
  std::unique_ptr<munin::layout> lyt =
      munin::make_grid_layout(terminalpp::extent{1, 1});
...
TEST(
    grid_layout_test,
    preferred_size_of_a_2x1_grid_with_two_views_is_the_twice_the_maximum_width_and_the_maximum_height)
{
  std::unique_ptr<munin::layout> lyt =
      munin::make_grid_layout(terminalpp::extent{2, 1});
  ...
  auto result = lyt->get_preferred_size({component0, component1}, {{}, {}});
...
TEST(
    grid_layout_test,
    preferred_size_of_a_1x2_grid_with_two_views_is_the_maximum_width_and_twice_the_maximum_height)
{
  std::unique_ptr<munin::layout> lyt =
      munin::make_grid_layout(terminalpp::extent{1, 2});
...
TEST(
    grid_layout_test,
    preferred_size_of_a_2x2_grid_with_four_views_is_twice_the_maximum_width_and_twice_the_maximum_height)
{
  std::unique_ptr<munin::layout> lyt =
      munin::make_grid_layout(terminalpp::extent{2, 2});
...
TEST(
    grid_layout_test,
    layout_of_1x1_grid_with_component_lays_out_component_to_that_size)
{
  std::unique_ptr<munin::layout> lyt =
      munin::make_grid_layout(terminalpp::extent{1, 1});
...
class laying_out_grids : public testing::TestWithParam<grid_layout_test_data>
{
};
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ValuesIn(std::vector<grid_layout_test_data>{
        grid_layout_test_data{
            {1, 1}, {10, 20}, {bounds_data{{0, 0}, {10, 20}}}},
        grid_layout_test_data{
            {2, 1},
            {4, 5},
            {bounds_data{{0, 0}, {2, 5}}, bounds_data{{2, 0}, {2, 5}}}},
        grid_layout_test_data{
            {1, 2},
            {5, 4},
            {bounds_data{{0, 0}, {5, 2}}, bounds_data{{0, 2}, {5, 2}}}},
        grid_layout_test_data{
            {3, 2},
            {9, 8},
            {bounds_data{{0, 0}, {3, 4}},
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ValuesIn(std::vector<grid_layout_test_data>{
        grid_layout_test_data{
            {2, 1},
            {11, 10},
            {bounds_data{{0, 0}, {6, 10}}, bounds_data{{6, 0}, {5, 10}}}},
        grid_layout_test_data{
            {1, 2},
            {10, 11},
            {bounds_data{{0, 0}, {10, 6}}, bounds_data{{0, 6}, {10, 5}}}},
        grid_layout_test_data{
            {3, 3},
            {14, 17},
            {bounds_data{{0, 0}, {5, 6}},
