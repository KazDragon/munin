...
TEST_F(
    ...
    calls_focus_previous_on_subcomponent_on_focus_previous)
{
...

    {
        ...
        EXPECT_CALL(*component_, do_focus_previous())
            .WillOnce(std::ref(component_->on_focus_set));
...
TEST_F(
    a_container_with_one_component,
    that_refuses_focus_refuses_focus_on_focus_previous)
{
    EXPECT_CALL(*component_, do_has_focus()).WillRepeatedly(Return(false));

    {
        InSequence s1;
        EXPECT_CALL(*component_, do_focus_previous());
        EXPECT_CALL(*component_, do_has_focus()).WillOnce(Return(false));
    }
...
TEST_F(
    ...
    skips_components_that_refuse_focus_previous_on_focus_previous)
{
...
TEST_F(
    ...
    does_not_yield_focus_on_previous_focus_when_the_component_does_not_yield_focus)
{
    {
        InSequence s1;
...
TEST_F(
    ...
    skips_components_that_refuse_focus_when_the_focused_component_yields_focus_on_previous_focus)
{
    {
        InSequence s1;
        EXPECT_CALL(*component2_, do_has_focus()).WillOnce(Return(true));
        EXPECT_CALL(*component2_, do_focus_previous())
            .WillOnce(std::ref(component2_->on_focus_lost));
        EXPECT_CALL(*component2_, do_has_focus()).WillOnce(Return(false));
...
TEST_F(
    a_container_with_two_components_where_the_last_has_focus,
    loses_focus_on_previous_focus_if_no_component_accepts_previous_focus)
{
    {
        InSequence s1;
        EXPECT_CALL(*component1_, do_has_focus()).WillOnce(Return(true));
