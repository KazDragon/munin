...
TEST_F(a_container_with_one_component, has_no_cursor)
{
    ...
    ASSERT_FALSE(container_.get_cursor_state());
    ASSERT_EQ(terminalpp::point(0, 0), container_.get_cursor_position());
}
    ...
    returns_the_state_and_position_of_the_focussed_component_relative_to_the_component_position)
{
    ...
    ASSERT_EQ(cursor_state, container_.get_cursor_state());
    ASSERT_EQ(expected_result, container_.get_cursor_position());
}
...
{
    EXPECT_CALL(*component0_, do_has_focus()).WillRepeatedly(Return(false));

    EXPECT_CALL(*component1_, do_has_focus()).WillRepeatedly(Return(true));

    EXPECT_CALL(*component1_, do_get_cursor_state())
        .WillRepeatedly(Return(true));

    EXPECT_CALL(*component1_, do_get_cursor_position())
    ...
    ASSERT_TRUE(container_.get_cursor_state());
    ASSERT_EQ(terminalpp::point(10, 12), container_.get_cursor_position());
}
    ...
    a_container_with_one_component, does_nothing_when_setting_cursor_position)
{
    ...
    container_.set_cursor_position({0, 0});
}
    ...
    sets_the_cursor_position_of_the_focussed_subcomponent_when_setting_cursor_position)
{
    ...
    EXPECT_CALL(*component_, do_set_cursor_position(cursor_position));

    container_.set_cursor_position(cursor_position);
}
    ...
    sets_the_cursor_position_of_the_focussed_subcomponent_relative_to_the_component_position)
{
    ...
    EXPECT_CALL(*component_, do_set_cursor_position(expected_position));
    ...
    skips_the_first_component_when_setting_cursor_position)
{
    ...
    EXPECT_CALL(*component1_, do_set_cursor_position(expected_position));
    ...
    does_not_emit_cursor_position_change_event_when_cursor_position_of_subcomponent_changes)
{
    ...
    component_->on_cursor_position_changed();
    ASSERT_EQ(0, cursor_position_changed_count_);
}
...
{
    EXPECT_CALL(*component0_, do_has_focus()).WillRepeatedly(Return(true));

    EXPECT_CALL(*component1_, do_has_focus()).WillRepeatedly(Return(false));

    component1_->on_cursor_position_changed();
    ASSERT_EQ(0, cursor_position_changed_count_);
}
    ...
    does_not_emit_cursor_state_changed_event_when_cursor_state_of_subcomponent_changes)
{
    ...
    component_->on_cursor_state_changed();
    ASSERT_EQ(0, cursor_state_changed_count_);
}
    ...
    does_not_emit_cursor_state_changed_event_when_unfocused_component_changes_cursor_state)
{
    ...
    component1_->on_cursor_state_changed();
    ASSERT_EQ(0, cursor_state_changed_count_);
}
...
{
    {
        InSequence s1;
        EXPECT_CALL(*component_, do_has_focus()).WillRepeatedly(Return(true));
        EXPECT_CALL(*component_, do_lose_focus()).Times(1);
    }
