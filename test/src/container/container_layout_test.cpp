...
TEST(
    a_container_with_no_elements,
    does_not_lay_the_container_out_when_a_component_is_added)
{
...
TEST(
    a_container_with_elements, lays_the_container_out_when_a_component_is_added)
{
    auto layout = make_mock_layout();
...
TEST_F(a_container, lays_out_the_container_when_a_component_is_added)
{
    ...
    EXPECT_CALL(*layout, do_layout(_, _, _)).Times(2);

    container_.set_size(size);
    container_.set_layout(std::move(layout));
    container_.add_component(component, hint);
}
