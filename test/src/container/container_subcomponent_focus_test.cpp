...
{
  {
    ...
    ON_CALL(*component0, do_has_focus()).WillByDefault(Return(true));
    ON_CALL(*component0, do_lose_focus())
        .WillByDefault(std::ref(component0->on_focus_lost));
    ON_CALL(*component0, do_has_focus()).WillByDefault(Return(false));
  }

  {
    testing::InSequence s2;
    ON_CALL(*component1, do_has_focus()).WillByDefault(Return(false));
    ON_CALL(*component1, do_set_focus())
        .WillByDefault(std::ref(component0->on_focus_set));
    ON_CALL(*component1, do_has_focus()).WillByDefault(Return(true));
  }
