    ...
    ForwardRange const &rng, terminalpp::point const &location)
{
  auto const &has_location_at_point = [&location](auto const &comp)
  {
    ...
    // bounds.
    return (
        location.x_ >= position.x_ && location.x_ < position.x_ + size.width_
...
struct container::impl
{
      ...
      std::shared_ptr<component> const &comp, boost::any const &layout_hint)
  {
    ...
    cnx.push_back(comp->on_redraw.connect(
        [this,
         wcomp = std::weak_ptr<component>(comp)](auto const &redraw_regions)
  ...
  void remove_component(std::shared_ptr<component> const &comp)
  {
    ...
    for (auto index = 0; index < components_.size(); ++index)
    {
      if (components_[index] == comp)
      {
        ...
        boost::for_each(component_connections_[index], disconnect_connection);

        component_connections_.erase(component_connections_.begin() + index);
      }
    }
  ...
  void lose_focus()
  {
    ...
    if (auto focussed_component = find_first_focussed_component(components_);
  ...
  // DRAW
  // ======================================================================
  void draw(render_surface &surface, terminalpp::rectangle const &region) const
  {
 ...
 private:
  ...
  void focus_incremental(ForwardRange const &components, Op &&increment_op)
  {
    ...
    auto const &increment_from = first_focussed_component == cend(components)
                                     ? cbegin(components)
  ...
  // ======================================================================
  void subcomponent_focus_set_handler(std::weak_ptr<component> const &weak_comp)
  {
    if (!in_focus_operation_)
    {
      ...
      if (auto comp = boost::find_if(components_, another_component_has_focus);
  ...
  {
    if (auto const &comp = find_component_at_point(components_, ev.position_);
        comp != components_.end())
    {
      auto const &position = (*comp)->get_position();
