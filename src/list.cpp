...
struct list::impl
{
  ...
  {
    if (auto const clicked_row = ev.position_.y_; clicked_row < items.size())
    {
  ...
  void handle_cursor_up()
  {
    if (!items.empty())
    {
      ...
      {
        self.select_item(std::optional(static_cast<int>(items.size() - 1)));
      }
    }
  }
...
void list::select_item(std::optional<int> const &index)
{
  ...
  assert(!index.has_value() || index < static_cast<int>(pimpl_->items.size()));
