...
{
  explicit used_headings(terminalpp::extent size = {}) : size_(std::move(size))
  {
  }

  terminalpp::coordinate_type north = 0;  // NOLINT
  terminalpp::coordinate_type south = 0;  // NOLINT
  terminalpp::coordinate_type east = 0;   // NOLINT
  terminalpp::coordinate_type west = 0;   // NOLINT
...
{
  auto const width =
      (std::min)(comp.get_preferred_size().width_, headings.remaining_width());
...
void layout_east_component(component &comp, used_headings &headings)
{
  ...
  comp.set_position(
      {(std::max)((headings.west + remaining_width) - preferred_size.width_, 0),
       headings.north});
    ...
    switch (hint)
    {
      ...
      case heading::centre:
        preferred_centre.width_ =
            std::max(preferred_centre.width_, comp_preferred_size.width_);
        preferred_centre.height_ =
            std::max(preferred_centre.height_, comp_preferred_size.height_);
        break;
      ...
      case heading::south:
        preferred_size.width_ +=
            std::max(comp_preferred_size.width_ - unused_size.width_, 0);
        unused_size.width_ +=
            std::max(comp_preferred_size.width_ - unused_size.width_, 0);

        preferred_size.height_ +=
            std::max(comp_preferred_size.height_ - unused_size.height_, 0);
        unused_size.height_ -=
