  ...
  auto get_preferred_size()
  {
    return terminalpp::extent{
        ...
        std::max(
            ...
            static_cast<terminalpp::coordinate_type>(laid_out_text_.size()))};
  }
 ...
 private:
  ...
  terminalpp::point clamp_cursor_position(terminalpp::point position)
  {
    ...
    position.y_ = boost::algorithm::clamp(
        ...
        static_cast<terminalpp::coordinate_type>(laid_out_text_.size() - 1));
    position.x_ = boost::algorithm::clamp(
        position.x_,
        terminalpp::coordinate_type{0},
        std::min(
            static_cast<terminalpp::coordinate_type>(std::max(width_ - 1, 0)),
  ...
  {
    set_cursor_position(
        {cursor_position_.x_, std::max(cursor_position_.y_ - repeat_count, 0)});
  }
  ...
  void handle_home(terminalpp::vk_modifier modifiers)
  {
    set_cursor_position(
        {0,
         modifiers == terminalpp::vk_modifier::ctrl ? 0 : cursor_position_.y_});
  }
  ...
  void handle_backspace()
  {
    if (caret_position_ != 0)
    {
      ...
      text_.erase(
          text_.begin() + caret_position_ - 1, text_.begin() + caret_position_);
