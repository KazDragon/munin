...
struct viewport::impl
{
  ...
  void draw(render_surface &surface, terminalpp::rectangle const &region)
  {
    ...
    surface.offset_by({-anchor_bounds_.origin_.x_, -anchor_bounds_.origin_.y_});
  ...
  auto event(boost::any const &ev)
  {
             ...
             keypress_event)
    {
      if (keypress_event->key == terminalpp::vk::pgup)
      {
        ...
        auto const cursor_position = tracked_component_->get_cursor_position();
 ...
 private:
      ...
      std::vector<terminalpp::rectangle> const &regions)
  {
    ...
    auto const translate_region = [this](terminalpp::rectangle const &region)
    {
        ...
        [viewport_size](terminalpp::rectangle const &region)
    {
      return terminalpp::rectangle{
          ...
          {std::min(
               region.size_.width_, viewport_size.width_ - region.origin_.x_),
           std::min(
